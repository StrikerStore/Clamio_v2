...
export const InventoryAggregation = forwardRef<InventoryAggregationRef, InventoryAggregationProps>(
  ({ onProductCountChange }, ref) => {
    ...
    useEffect(() => {
      fetchInventory();
      fetchRTOInventory(); // Auto-fetch RTO data from database
    }, []);
    ...
    /**
     * Fetch RTO inventory from database API
     */
    const fetchRTOInventory = async () => {
      ...
      try {
        ...
        if (data.success) {
          setRTOData(data.data.rtoData);
          console.log(`✅ RTO Data loaded: ${data.data.totalEntries} entries`);
        } else {
          throw new Error(data.error || "Failed to fetch RTO inventory");
        }
      } catch (error) {
        console.error("Error fetching RTO inventory:", error);
        // Don't show toast for RTO fetch failure - it's not critical
        console.log("RTO data fetch failed, continuing without RTO data");
      } finally {
        setRtoLoading(false);
      }
    };

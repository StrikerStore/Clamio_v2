...
class ApiClient {
  ...
  // Admin label download methods
  async downloadLabelAsAdmin(orderId: string, format: string = 'thermal'): Promise<ApiResponse> {
    return this.makeRequest('/orders/admin/download-label', {
      method: 'POST',
      body: JSON.stringify({ order_id: orderId, format })
    });
  }

  async bulkDownloadLabelsAsAdmin(orderIds: string[], format: string = 'thermal'): Promise<Blob> {
    const authHeader = typeof window !== 'undefined' ? localStorage.getItem('authHeader') : null;
    
    if (!authHeader) {
      throw new Error('Admin authentication required');
    }

    console.log('🔵 ADMIN BULK DOWNLOAD LABELS API REQUEST START');
    console.log('  - orderIds:', orderIds);
    console.log('  - format:', format);
    console.log('  - authHeader:', authHeader ? authHeader.substring(0, 20) + '...' : 'null');
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/orders/admin/bulk-download-labels`, config);
      
      console.log('📥 ADMIN BULK DOWNLOAD LABELS API RESPONSE RECEIVED');
      ...
      if (contentType && contentType.includes('application/pdf')) {
        // Single vendor PDF response
        const blob = await response.blob();
        console.log('  - Blob size:', blob.size);
        console.log('  - Blob type:', blob.type);
        return blob;
      } else {
        // Multiple vendors JSON response - handle multiple PDF downloads
        const data = await response.json();
        console.log('  - JSON response:', data);
        
        if (data.data && data.data.vendor_groups && data.data.vendor_groups.length > 0) {
          console.log(`📥 ADMIN: Processing ${data.data.vendor_groups.length} vendor groups`);
          
          // Process PDFs for each vendor group
          const downloadPromises = data.data.vendor_groups.map(async (vendorGroup: any) => {
            try {
              console.log(`📥 ADMIN: Processing PDF for vendor ${vendorGroup.vendor_name}`);
              
              // Convert base64 to blob
              const binaryString = atob(vendorGroup.pdf_data);
              ...
              console.log(`✅ ADMIN: Created PDF for ${vendorGroup.vendor_name} (${blob.size} bytes)`);
              
              // Create download link
              const url = window.URL.createObjectURL(blob);
              const link = document.createElement('a');
              link.href = url;
              link.download = vendorGroup.pdf_filename;
              ...
              return {
                ...
                order_count: vendorGroup.order_count,
                success: true
              };
              
            } catch (error) {
              console.error(`❌ ADMIN: Error processing PDF for ${vendorGroup.vendor_name}:`, error);
              return {
                vendor_name: vendorGroup.vendor_name,
                order_count: vendorGroup.order_count,
                success: false,
          ...
          console.log(`✅ ADMIN: Download complete - ${successful.length} successful, ${failed.length} failed`);
          
          // Return a special blob that indicates multiple downloads
          const summaryBlob = new Blob([JSON.stringify({
            type: 'multiple_vendor_downloads',
            successful: successful,
            failed: failed,
            total_vendors: data.data.vendor_groups.length
      ...
      console.error('Admin bulk download labels API request failed:', error);
      throw error;
    }
  }
